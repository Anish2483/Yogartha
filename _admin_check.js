...
// ============ YOGARTHA ADMIN v7 â€” Mobile + Desktop ============
...
let schedule = {
 ...
 {day:"Sunday", slots:["Deep Meditation","Hatha Yoga","â€”","â€”"]}
 ],
...
function testImgbbKey() {
 ...
 res.style.color="var(--muted)"; res.textContent="Testingâ€¦";
 ...
 if(d.success){res.style.color="#6ee89a";res.textContent="âœ“ Connected! Uploads enabled.";updateImgbbStatus();}
 else{res.style.color="#f87171";res.textContent="âœ— Invalid key. Please check.";}
 }).catch(()=>{res.style.color="#f87171";res.textContent="âœ— Network error.";});
}
function updateImgbbStatus() {
 ...
 if(k){bar.className="imgbb-bar ready";msg.textContent="âœ“ ImgBB connected â€” direct photo upload enabled!";}
 else{bar.className="imgbb-bar notset";msg.textContent='ImgBB not set â€” go to "Image API Setup" to enable direct uploads.';}
...
async function uploadToImgbb(file, onProgress) {
 ...
 if(file.size>10*1024*1024){toast("File too large â€” max 10MB",true);return null;}
...
function doLogin() {
 ...
 try {
  if(typeof firebase!=="undefined"&&firebase.apps.length>0){
   ...
   err.textContent="Security Error: Firebase Authentication failed to initialize. Please check your connection.";
  }
 } catch(e) {
...
function enterDash(fb) {
 ...
 bar.innerHTML='<div class="status-dot"></div><span>Connected to Firebase â€” changes publish live instantly</span>';
 loadFirebase();
 }
...
function loadEnquiries() {
 ...
 list.innerHTML = '<p style="color:var(--muted);font-size:.88rem;">Loadingâ€¦</p>';
 ...
 <div style="font-weight:600;font-size:.95rem;color:var(--text);margin-bottom:4px;">${e.name || 'â€”'}</div>
 ...
 ${e.email ? `<span>âœ‰ ${e.email}</span>` : ''}
 ${e.phone ? `<span>ðŸ“ž ${e.phone}</span>` : ''}
 ${e.program ? `<span>ðŸ§˜ ${e.program}</span>` : ''}
 </div>
...
function _hideAuthGate() {
 const gate = document.getElementById("auth-loading");
...
document.addEventListener("DOMContentLoaded", () => {
 ...
 // ---- SECURE AUTH GATE ----
 // Only Firebase can open the dashboard. The loading screen hides everything
 // until onAuthStateChanged fires. If not logged in, show login. If logged in,
 // go straight to dashboard. No bypass is possible.
 if (typeof firebase !== "undefined" && firebase.apps.length > 0) {
  // Use SESSION persistence â€” login won't carry over after browser is fully closed
   ...
   .then(() => {
    firebase.auth().onAuthStateChanged(user => {
     ...
     if (user) {
      ...
      // Not logged in â€” show the login screen
      document.getElementById("login-screen").style.display = "flex";
     }
    });
   })
   .catch(() => {
    // setPersistence failed (e.g. private browsing) â€” still gate properly
    firebase.auth().onAuthStateChanged(user => {
     _hideAuthGate();
     if (user) {
      enterDash(true);
  ...
  // Firebase failed to load entirely â€” show error on login screen, never open dashboard
  _hideAuthGate();
  document.getElementById("login-screen").style.display = "flex";
  document.getElementById("login-err").textContent = "Firebase failed to initialize. Check your connection or disable adblockers.";
 }
});
...
function addBatch() {
  ...
  if (isFirebase) {
    ...
    toast('Batch added (Demo mode â€” connect Firebase to publish)');
    resetBatchForm();
  }
}
    ...
    return `<div style="display:flex;align-items:center;gap:12px;padding:14px 0;border-bottom:1px solid var(--border);">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:.78rem;color:var(--muted);margin-top:2px;">
          ${fmtDateAdmin(b.startDate)} â†’ ${fmtDateAdmin(b.endDate)} &nbsp;Â·&nbsp; ${days} days
          ${b.time ? '&nbsp;Â·&nbsp; ' + b.time : ''}
          ${b.seats ? '&nbsp;Â·&nbsp; ' + b.seats : ''}
        </div>
      </div>
...
async function uploadImageFile(file, _storagePath) {
  ...
  if (!k) {
    ...
    toast('âš  Paste your ImgBB API key in "Image API Setup" to enable uploads', true);
...
async function handleGalleryFiles(files){
  ...
  async function cropAndUploadNext(index) {
    ...
    openCropModal(file, `Crop photo ${index+1} of ${fileArray.length} â€” drag to adjust`, async (blob) => {
      ...
      progText.textContent = `Uploading ${index+1}/${fileArray.length}â€¦`;
      try {
...
function saveGallery(){
 ...
 else toast("Demo mode â€” connect Firebase to publish");
}
...
async function handleTestiFile(file){
  if(!file)return;
  // Open crop modal â€” square/round for portrait photo
  openCropModal(file, 'Crop student photo â€” square works best for the avatar', async (blob) => {
    const croppedFile = blobToFile(blob, file.name);
    if(!getImgbbKey()){
    ...
    prog.classList.add("show"); progText.textContent="Uploadingâ€¦";
    try{
      const url=await uploadToImgbb(croppedFile,pct=>{progBar.style.width=pct+"%";progText.textContent=`Uploadingâ€¦ ${pct}%`;});
...
async function uploadSiteImg(type, file, previewId) {
  ...
  const cropConfigs = {
    hero:     { ratio: 16/9, label: 'Crop hero background â€” 16:9 wide recommended', isRound: false },
    about:    { ratio: NaN,  label: 'Crop about section image',                      isRound: false },
    guru:     { ratio: 1,    label: 'Crop teacher portrait â€” square works best',    isRound: true  },
    sadhguru: { ratio: NaN,  label: 'Crop Sadhguru portrait',                       isRound: false },
    shala:    { ratio: NaN,  label: 'Crop class / studio photo',                    isRound: false },
    symbol:   { ratio: 1,    label: 'Crop decorative symbol â€” square recommended', isRound: false },
  };
  ...
  openCropModal(file, cfg.label, async (fileOrBlob) => {
    ...
    try {
      ...
      if (url) {
        ...
        toast('Image updated live! âœ“');
      }
    } catch(e) {
      toast('Upload failed â€” ' + e, true);
    } finally {
  ...
  openCropModal(file, cfg.label, async (fileOrBlob) => {
    ...
    try {
      ...
      if (url) {
        ...
        toast('Hero ' + device + ' image updated live! âœ“');
      }
    } catch(e) { toast('Upload failed â€” ' + e, true); }
  }, cfg.ratio, false);
}
  ...
  openCropModal(file, cfg.label, async (fileOrBlob) => {
    ...
    try {
      ...
      if (url) {
        ...
        toast('Program hero ' + device + ' updated live! ðŸŽ‰');
      }
    } catch(e) { toast('Upload failed â€” ' + e, true); }
  }, cfg.ratio, false);
}
  ...
  if (pId && isFirebase) {
    ...
    firebase.database().ref('programs/' + pId).once('value').then(s => {
      ...
      if (data.regFormUrl && regInput) {
        ...
        if (regStatus) { regStatus.style.color = '#43A047'; regStatus.textContent = 'âœ“ Form link active'; }
      }
    }).catch(() => {});
  }
}
...
function saveProgRegFormUrl() {
  ...
  if (isFirebase) {
    firebase.database().ref('programs/' + pId + '/regFormUrl').set(url)
      .then(() => {
        if (status) { status.style.color = '#43A047'; status.textContent = 'âœ“ Form link saved & live!'; }
  ...
  if (isFirebase) {
    firebase.database().ref('programs/' + pId + '/regFormUrl').remove()
      .then(() => {
        ...
        if (status) { status.style.color = 'var(--muted)'; status.textContent = 'Form link removed â€” button hidden on program page.'; }
...
async function uploadProgramImg(type, file, previewId) {
  ...
  openCropModal(file, cropLabel, async (fileOrBlob) => {
    const uploadFile = (fileOrBlob instanceof File) ? fileOrBlob : blobToFile(fileOrBlob, file.name);
    toast('Uploading program imageâ€¦');
    try {
      ...
      if (url) {
        ...
        toast('Program image updated! âœ“');
      }
...
function saveAnnouncement(){
 ...
 else toast("Demo mode â€” connect Firebase");
}
